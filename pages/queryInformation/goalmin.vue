        ...
        <div class="content">
            <div class="subsection">
                <div style="margin: 25px 10px;">
                    ...
                    <div style="margin: 25px 10px;">
                        <span style="vertical-align: middle;">Team with MIN average goals: </span>
                    </div>
                    <ul style="list-style-type: none; padding: 0; margin: 0;">
                        <li v-for="(team, index) in teams" :key="index" style="padding: 10px 20px; margin: 0 25px; position: relative;">
                            {{ 'Team: ' + team.name + ',  Average Goals: '+ team.min_avg_goals}}
                        </li>
                    </ul>
                </div>
            </div>
...
<script>
    ...
    export default {
      async asyncData () {
        let { data } = await axios.get('/api/nestedAggregation/min')
        return { teams: data }
      },
